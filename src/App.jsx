...
const INTERVALS       = ["Diario","Semanal","Quincenal","Mensual","Trimestral","Semestral","Anual","Por horas","Por millas","Una sola vez"];
...
const INIT_VESSELS = [
  {
    ...
    tasks:[
      ...
      {id:2,systemId:"casco",system:"Casco",equipment:"Casco Fondo",name:"Pulida y encerado",assigned:"Carlos Mendoza",interval:"Semestral",nextDue:"2025-06-19",status:"due",notes:"Cera marina UV. 2 capas."},
      {id:3,systemId:"agua",system:"Sistema de Agua",equipment:"Desalinizadora / Watermaker",name:"Service semanal watermaker",assigned:"Carlos Mendoza",interval:"Semanal",nextDue:"2025-05-30",status:"overdue",notes:"Flush con agua limpia post-uso."},
    ...
    records:[
      ...
      {id:2,date:"2023-03-23",type:"Preventivo",systemId:"casco",equipment:"Casco Costados",hours:"N/A",desc:"Encerado y pulida de costados",performedBy:"Omikron",cost:1000},
      ...
      {id:7,date:"2023-05-16",type:"Reactivo",systemId:"ac",equipment:"A/C Camarote Principal",hours:"N/A",desc:"Bomba A/C reventada con fuga",performedBy:"The Boating Zone",cost:1397},
      {id:8,date:"2023-08-16",type:"Reactivo",systemId:"ac",equipment:"Evaporador",hours:"N/A",desc:"Ambos evaporadores con fuga, bandeja rota",performedBy:"Quality Air Marine",cost:14995},
    ],
    docs:12, alerts:2,
...
export default function App() {
  ...
  const updateVessel = (updated) => setVessels(vs=>vs.map(v=>v.id===updated.id?updated:v));

  return (
    <div style={s.root} onClick={()=>{setShowVesselMenu(false);setShowUserMenu(false);}}>
      <TopNav
        vessel={vessel} vessels={vessels}
          ...
          {showUserMenu&&(
            <div style={{...s.drop,minWidth:210}}>
              ...
              {[
                ...
                {icon:"🔒",label:"Seguridad",         action:()=>setShowUserMenu(false)},
...
function HomePage({vessel,setPage,vessels}) {
  const upcoming = [...vessel.tasks].filter(t=>t.status!=="done").sort((a,b)=>{
    const da=new Date(a.nextDue),db=new Date(b.nextDue); return da-db;
  }).slice(0,5);
  return (
    <div style={s.home}>
      <div style={s.row}>
    ...
    <div style={{...s.card,flex:1}}>
      ...
      {vessel.log.slice(0,4).map((e,i)=>(
        <div key={i} style={s.logRow}>
          ...
          <span style={{fontSize:12,color:"#334155",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.type==="Salida"?`${e.dest} · ${e.persons}p`:e.type==="Compra"?e.item:(e.desc||"").slice(0,50)}</span>
  ...
  return (
    <div style={{...s.card,display:"flex",alignItems:"center",gap:24,padding:"14px 24px",flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:20,flex:1,flexWrap:"wrap"}}>
        {[["💨",`${w.wind} kn`,"Viento"],["🌊","0.4 m","Oleaje"],["💧","78%","Humedad"],["☀️","UV 7","Índice UV"],["🌡️",`${w.temp}°C`,"Temperatura"]].map(([ic,val,lbl])=>(
      ...
      <div style={{fontSize:10,color:"#94a3b8",flexShrink:0}}>Actualizado hace 5 min</div>
    </div>
  );
}
...
function TasksPage({vessel,updateVessel}) {
  ...
  const FILTERS = ["Todas","Vencidas","Por vencer","Completadas"];
  ...
  const addTask = (task) => {
    updateVessel({...vessel, tasks:[...vessel.tasks,{...task,id:Date.now()}]});
    setShowAdd(false);
  };
  ...
  return (
    <div style={{padding:"24px 28px"}}>
        ...
        <table style={s.table}>
          <thead>
            <tr style={{background:"#1e3a5f"}}>
              {["Sistema","Equipo","Tarea","Asignado","Intervalo","Próximo Venc.","Estado","Notas"].map(h=><th key={h} style={s.th}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            ...
            {filtered.map(task=>{
              ...
              const sys=SYSTEMS.find(x=>x.id===task.systemId);
              return (
                <>
                    ...
                    <tr key={`e${task.id}`}>
                      <td colSpan={8} style={{padding:0,background:"#f0f7ff",borderBottom:"2px solid #bfdbfe"}}>
                        <div style={{padding:"16px 24px",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                          {[{l:"Sistema",v:task.system},{l:"Equipo",v:task.equipment},{l:"Intervalo",v:task.interval},{l:"Próx. Vencimiento",v:task.nextDue},{l:"Asignado a",v:task.assigned},{l:"Estado",v:p.l},{l:"Notas",v:task.notes||"—"},{l:"Fotos",v:(task.photos||[]).length>0?`${task.photos.length} foto(s)`:"Sin fotos"}].map((c,i)=>(
      ...
      {showAdd&&<AddTaskModal vessel={vessel} onSave={addTask} onClose={()=>setShowAdd(false)}/>}
    </div>
  );
}

// ── ADD TASK MODAL ────────────────────────────────────────────────────────────
function AddTaskModal({vessel,onSave,onClose}) {
  ...
  const handleSave = () => {
    if(!validate()) return;
    const today=new Date(), nd=new Date(nextDue);
    const diff=Math.round((nd-today)/(1000*60*60*24));
    let status="ok";
    if(diff<0) status="overdue";
    else if(diff<=14) status="due";
    onSave({systemId,system:selectedSystem?.label||systemId,equipment,name,assigned,interval,nextDue,status,notes,photos:[]});
  };
      ...
      <div style={{...s.modalBox,maxWidth:560}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          {selectedSystem&&(
            <div>
              ...
              <select value={equipment} onChange={e=>setEquipment(e.target.value)} style={{...s.input,borderColor:errors.equipment?"#dc2626":"#e2e8f0"}}>
                ...
                {selectedSystem.equipment.map(eq=><option key={eq} value={eq}>{eq}</option>)}
              </select>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Observaciones, materiales necesarios, instrucciones..." rows={3} style={{...s.input,resize:"vertical"}}/>
          </div>
        </div>
          ...
          <tbody>
            ...
            {filtered.map((e,i)=>(
              <tr key={i} style={s.trow}>
                ...
                <td style={s.td}>
                  <span style={{background:LOG_COLOR[e.type]+"18",color:LOG_COLOR[e.type],padding:"2px 10px",borderRadius:20,fontSize:11,fontWeight:600}}>{e.type}{e.serviceType?` · ${e.serviceType}`:""}</span>
                </td>
                <td style={{...s.td,color:"#334155",maxWidth:360}}>
                  {e.type==="Salida"?`${e.dest||""} · ${e.persons||""}p · Salida ${e.deptTime||"—"} Llegada ${e.arrTime||"Pendiente"}`:e.type==="Compra"?`${e.item||""} · $${e.costUSD||0} · ${e.payment||""}`:e.desc||""}
                </td>
...
function LogEntryModal({vessel,initial,onSave,onClose}) {
  ...
  const providerNames  = vessel.providers.map(p=>`${p.firstName} ${p.lastName} (${p.company})`);
  const allPerformedBy = [vessel.captain,...providerNames];
  const selectedSys    = SYSTEMS.find(s=>s.id===systemId);
  const needsHours     = selectedSys?.trackHours && equipment;
  ...
  const handleSave=()=>{
    ...
    let entry={...base};
    if(type==="Servicio")    entry={...entry,serviceType,systemId,equipment:selectedSys?.label,equipHours:needsHours?equipHours:null};
  ...
  const Fld=({label,req,err,children})=>(
    <div><label style={s.label}>{label}{req&&<span style={{color:"#dc2626"}}> *</span>}</label>{children}{err&&<div style={s.errMsg}>{err}</div>}</div>
  );
    ...
    <div style={s.modalOverlay} onClick={onClose}>
      <div style={{...s.modalBox,maxWidth:580}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
          ...
          <Fld label="Tipo" req err={errors.type}>
          ...
          <Fld label="Fecha" req err={errors.date}>
          ...
          {type==="Inspección"&&(<>
            <Fld label="Sistema">
              <select value={systemId} onChange={e=>{setSystemId(e.target.value);setEquipment("");}} style={s.input}>
                ...
                {SYSTEMS.map(sys=><option key={sys.id} value={sys.id}>{sys.icon} {sys.label}</option>)}
              </select>
            </Fld>
            {systemId&&<Fld label="Equipo"><select value={equipment} onChange={e=>setEquipment(e.target.value)} style={s.input}><option value="">Seleccionar...</option>{selectedSys?.equipment.map(eq=><option key={eq} value={eq}>{eq}</option>)}</select></Fld>}
            <Fld label="Descripción" req err={errors.desc}><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={3} style={{...s.input,resize:"vertical"}}/></Fld>
            <Fld label="Realizado por"><select value={performedBy} onChange={e=>setPerformedBy(e.target.value)} style={s.input}><option value="">Seleccionar...</option>{allPerformedBy.map(p=><option key={p} value={p}>{p}</option>)}</select></Fld>
            <PhotoFld count={photos} setCount={setPhotos} err={errors.photos}/>
          </>)}

          {type==="Servicio"&&(<>
            <Fld label={<>Tipo de Servicio <span style={{position:"relative",display:"inline-block"}}><span style={{cursor:"pointer",color:"#94a3b8",fontSize:12}} onClick={()=>setShowInfo(!showInfo)}>ⓘ</span>{showInfo&&<div style={{position:"absolute",left:0,top:"100%",zIndex:10,background:"#1e293b",color:"#fff",fontSize:11,borderRadius:8,padding:"10px 12px",width:220,lineHeight:1.6,boxShadow:"0 4px 16px rgba(0,0,0,0.2)"}}><b>Preventivo:</b> Mantenimiento planificado.<br/><b>Reactivo:</b> Respuesta a falla inesperada.<br/><b>Reparación:</b> Arreglo de algo dañado.</div>}</span></>} req err={errors.serviceType}>
            ...
            <Fld label="Sistema" req err={errors.system}>
              <select value={systemId} onChange={e=>{setSystemId(e.target.value);setEquipment("");}} style={s.input}>
                <option value="">Seleccionar sistema...</option>
                {SYSTEMS.map(sys=><option key={sys.id} value={sys.id}>{sys.icon} {sys.label}</option>)}
              </select>
            </Fld>
            {systemId&&<Fld label="Equipo" req err={errors.equipment}><select value={equipment} onChange={e=>setEquipment(e.target.value)} style={{...s.input,borderColor:errors.equipment?"#dc2626":"#e2e8f0"}}><option value="">Seleccionar...</option>{selectedSys?.equipment.map(eq=><option key={eq} value={eq}>{eq}</option>)}</select></Fld>}
            {needsHours&&<Fld label={`Horas actuales de ${equipment}`} req><input type="number" value={equipHours} onChange={e=>setEquipHours(e.target.value)} placeholder="Ej: 1245" style={s.input}/></Fld>}
            <Fld label="Descripción" req err={errors.desc}><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={3} style={{...s.input,resize:"vertical"}}/></Fld>
            <Fld label="Realizado por"><select value={performedBy} onChange={e=>setPerformedBy(e.target.value)} style={s.input}><option value="">Seleccionar...</option>{allPerformedBy.map(p=><option key={p} value={p}>{p}</option>)}</select></Fld>
            <PhotoFld count={photos} setCount={setPhotos} err={errors.photos}/>
          </>)}
          ...
          {type==="Combustible"&&(<>
            <Fld label="Nivel actual de combustible" req err={errors.fuelQty}>
              <div style={{display:"flex",gap:10}}>
                ...
                <div style={{flex:1}}><label style={{...s.label,marginBottom:4}}>Cantidad</label><input type="number" value={fuelQty} onChange={e=>setFuelQty(e.target.value)} placeholder={`Ej: 280`} style={{...s.input,borderColor:errors.fuelQty?"#dc2626":"#e2e8f0"}}/></div>
              </div>
            </Fld>
            <Fld label="Comentarios"><textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Proveedor, tipo de combustible..." rows={2} style={{...s.input,resize:"vertical"}}/></Fld>
            <Fld label="Realizado por"><select value={performedBy} onChange={e=>setPerformedBy(e.target.value)} style={s.input}><option value="">Seleccionar...</option>{allPerformedBy.map(p=><option key={p} value={p}>{p}</option>)}</select></Fld>
          </>)}

          {type==="Salida"&&(<>
            <div style={{background:"#f0f9ff",borderRadius:10,padding:"14px 16px",border:"1px solid #bae6fd"}}>
              <div style={{fontWeight:700,fontSize:12,color:"#0369a1",marginBottom:12}}>🚢 INFORMACIÓN DE SALIDA</div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <Row2><Fld label="Destino" req><input value={dest} onChange={e=>setDest(e.target.value)} style={s.input}/></Fld><Fld label="Clima"><input value={clima} onChange={e=>setClima(e.target.value)} placeholder="Ej: Soleado, 12kn" style={s.input}/></Fld></Row2>
                <Row2><Fld label="Hora de Salida"><input type="time" value={deptTime} onChange={e=>setDeptTime(e.target.value)} style={s.input}/></Fld><Fld label="N° Personas"><input type="number" value={persons} onChange={e=>setPersons(e.target.value)} style={s.input}/></Fld></Row2>
                ...
                <Row2><Fld label={`Combustible al salir (${vessel.fuelUnit})`}><input type="number" value={fuelOut} onChange={e=>setFuelOut(e.target.value)} style={s.input}/></Fld><Fld label="Horas Motor al salir"><input type="number" value={engOut} onChange={e=>setEngOut(e.target.value)} style={s.input}/></Fld></Row2>
                <Fld label="Horas Generador al salir"><input type="number" value={genOut} onChange={e=>setGenOut(e.target.value)} style={s.input}/></Fld>
              </div>
            </div>
            <div style={{background:"#f0fdf4",borderRadius:10,padding:"14px 16px",border:"1px solid #bbf7d0"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <Row2><Fld label="Hora de Llegada"><input type="time" value={arrTime} onChange={e=>setArrTime(e.target.value)} style={s.input}/></Fld><Fld label={`Combustible al regresar (${vessel.fuelUnit})`}><input type="number" value={fuelIn} onChange={e=>setFuelIn(e.target.value)} style={s.input}/></Fld></Row2>
                <Row2><Fld label="Horas Motor al regresar"><input type="number" value={engIn} onChange={e=>setEngIn(e.target.value)} style={s.input}/></Fld><Fld label="Horas Generador al regresar"><input type="number" value={genIn} onChange={e=>setGenIn(e.target.value)} style={s.input}/></Fld></Row2>
              </div>
            </div>
            <Fld label="Observaciones"><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} style={{...s.input,resize:"vertical"}}/></Fld>
            <PhotoFld count={photos} setCount={setPhotos}/>
          </>)}

          {type==="Compra"&&(<>
            <Fld label="Artículo" req err={errors.item}><input value={item} onChange={e=>setItem(e.target.value)} placeholder="Ej: Filtro aceite Fleetguard FF5052" style={{...s.input,borderColor:errors.item?"#dc2626":"#e2e8f0"}}/></Fld>
            <Fld label="Descripción"><textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} style={{...s.input,resize:"vertical"}}/></Fld>
            <Row2>
              <Fld label="Costo USD"><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{color:"#64748b"}}>$</span><input type="number" value={costUSD} onChange={e=>setCostUSD(e.target.value)} placeholder="0.00" style={s.input}/></div></Fld>
              <Fld label="Costo Bs"><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{color:"#64748b"}}>Bs</span><input type="number" value={costBs} onChange={e=>setCostBs(e.target.value)} placeholder="0.00" style={s.input}/></div></Fld>
            </Row2>
            <Fld label="Método de Pago"><select value={payment} onChange={e=>setPayment(e.target.value)} style={s.input}><option value="">Seleccionar...</option>{PAYMENT_METHODS.map(p=><option key={p} value={p}>{p}</option>)}</select></Fld>
...
function RecordsPage({vessel}) {
  ...
  const salidaLog   = vessel.log.filter(e=>e.type==="Salida");

  const filteredService = subFilter==="Todos" ? serviceLog : serviceLog.filter(e=>e.serviceType===subFilter);

  const totalCost = vessel.records.reduce((a,r)=>a+r.cost,0);
  const totalPurchases = purchaseLog.reduce((a,e)=>a+(e.costUSD||0),0);
  const totalFuel = fuelLog.length;
  ...
  return (
    <div style={{padding:"24px 28px"}}>
      ...
      <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}>
        {[
          {label:"Servicios",val:serviceLog.length,color:"#2563eb",sub:"en bitácora"},
          {label:"Inspecciones",val:inspLog.length,color:"#16a34a",sub:"en bitácora"},
          {label:"Combustible",val:totalFuel,color:"#d97706",sub:"repostajes"},
          {label:"Compras",val:purchaseLog.length,color:"#0891b2",sub:`$${totalPurchases.toFixed(2)}`},
          {label:"Mantenimiento Hist.",val:vessel.records.length,color:"#7c3aed",sub:`$${totalCost.toLocaleString("en-US",{minimumFractionDigits:2})}`},
        ].map(item=>(
          <div key={item.label} style={{padding:"12px 16px",background:"#fff",border:"1px solid #e2e8f0",borderRadius:10,flex:1,minWidth:120,textAlign:"center",cursor:"pointer"}} onClick={()=>setMainFilter(item.label==="Mantenimiento Hist."?"Historico":item.label.split(" ")[0])}>
      ...
      {mainFilter==="Servicio"&&(
        <>
          <div style={{display:"flex",gap:6,marginBottom:12}}>
            {["Todos","Preventivo","Reactivo","Reparación"].map(f=>(
              <button key={f} onClick={()=>setSubFilter(f)} style={{...s.filterBtn,background:subFilter===f?"#1e3a5f":"transparent",color:subFilter===f?"#fff":"#64748b",borderColor:subFilter===f?"#4a9eff":"#e2e8f0",fontSize:11}}>{f}</button>
            ))}
          </div>
          <RecordTable rows={filteredService.map(e=>({date:e.date,type:e.serviceType||"—",equip:e.equipment||"—",desc:e.desc,by:e.performedBy,photos:(e.photos||[]).length}))} cols={["Fecha","Tipo","Equipo","Descripción","Realizado por","Fotos"]}/>
        </>
      )}

      {/* Inspecciones */}
      {mainFilter==="Inspecciones"&&(
        <RecordTable rows={inspLog.map(e=>({date:e.date,equip:e.equipment||"—",desc:e.desc,by:e.performedBy,photos:(e.photos||[]).length}))} cols={["Fecha","Equipo","Descripción","Realizado por","Fotos"]}/>
      )}

      {/* Combustible */}
      {mainFilter==="Combustible"&&(
        <RecordTable rows={fuelLog.map(e=>({date:e.date,qty:`${e.fuelQty} ${e.fuelUnit}`,desc:e.desc||"—",by:e.performedBy}))} cols={["Fecha","Cantidad","Notas","Registrado por"]}/>
      )}

      {/* Compras */}
      {mainFilter==="Compras"&&(
        <>
          <div style={{marginBottom:12,padding:"10px 16px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,display:"flex",gap:16}}>
            <span style={{fontSize:13,color:"#64748b"}}>Total compras:</span>
            <span style={{fontWeight:700,color:"#16a34a"}}>${totalPurchases.toFixed(2)} USD</span>
          </div>
          <RecordTable rows={purchaseLog.map(e=>({date:e.date,item:e.item,desc:e.desc||"—",usd:`$${e.costUSD||0}`,bs:e.costBs?`Bs ${e.costBs}`:"—",payment:e.payment||"—",photos:(e.photos||[]).length}))} cols={["Fecha","Artículo","Descripción","USD","Bs","Pago","Fotos"]}/>
        </>
      )}
      ...
      {mainFilter==="Historico"&&(
        <>
          <div style={{marginBottom:12,padding:"10px 16px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,display:"flex",gap:16}}>
            <span style={{fontSize:13,color:"#64748b"}}>Costo total histórico:</span>
            <span style={{fontWeight:700,color:"#16a34a"}}>${totalCost.toLocaleString("en-US",{minimumFractionDigits:2})}</span>
          </div>
          <RecordTable rows={vessel.records.map(r=>({date:r.date,type:r.type,equip:r.equipment,hours:r.hours,desc:r.desc,by:r.performedBy,cost:`$${r.cost.toLocaleString("en-US",{minimumFractionDigits:2})}`}))} cols={["Fecha","Tipo","Equipo","Horas","Descripción","Realizado por","Costo"]}/>
        </>
      )}
...
function RecordTable({rows,cols}) {
  if(rows.length===0) return <div style={{textAlign:"center",padding:"40px",color:"#94a3b8"}}>Sin registros en esta categoría</div>;
      ...
      <table style={s.table}>
        ...
        <tbody>
          {rows.map((row,i)=>(
            <tr key={i} style={s.trow}>
              {keys.map((k,j)=>(
                <td key={j} style={{...s.td,whiteSpace:k==="date"?"nowrap":"normal",color:k==="cost"?"#16a34a":k==="usd"?"#16a34a":"#475569",fontWeight:k==="cost"||k==="usd"?600:400,maxWidth:k==="desc"?300:"none"}}>
                  {k==="photos"?row[k]>0?`📷 ${row[k]}`:"—":row[k]}
                </td>
              ))}
            </tr>
          ))}
...
function ReportModal({vessel,onClose}) {
  ...
  const types=[
    {key:"costs",    icon:"💰",label:"Reporte de Costos",       desc:"Vinculado a compras y mantenimiento"},
    {key:"salidas",  icon:"🚢",label:"Reporte de Salidas",       desc:"Todas las salidas al mar"},
    {key:"fuel",     icon:"⛽",label:"Reporte de Combustible",   desc:"Historial de repostajes"},
    {key:"service",  icon:"🔧",label:"Reporte de Servicios",     desc:"Preventivo, reactivo y reparaciones"},
    {key:"inspect",  icon:"🔍",label:"Reporte de Inspecciones",  desc:"Todas las inspecciones realizadas"},
    {key:"log",      icon:"📓",label:"Bitácora Completa",        desc:"Todas las entradas"},
    {key:"tasks",    icon:"☑", label:"Estado de Tareas",         desc:"Completadas, pendientes y vencidas"},
    {key:"sale",     icon:"⚓", label:"Reporte para Venta",       desc:"Historial completo para compradores"},
    {key:"insurance",icon:"🛡", label:"Reporte para Seguro",     desc:"Mantenimientos y estado general"},
  ];
        ...
        {!gen?(
          <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:16}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {types.map(r=>(
                <button key={r.key} onClick={()=>toggle(r.key)} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",border:"1.5px solid",borderRadius:8,cursor:"pointer",textAlign:"left",background:sel.includes(r.key)?"#eff6ff":"#f8fafc",borderColor:sel.includes(r.key)?"#2563eb":"#e2e8f0"}}>
                  ...
                  {sel.includes(r.key)&&<span style={{color:"#2563eb",fontSize:14,flexShrink:0}}>✓</span>}
                </button>
              ))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div style={{padding:"10px 14px",background:"#fefce8",border:"1px solid #fde68a",borderRadius:8,fontSize:12,color:"#92400e"}}>
              💡 El <b>Reporte para Venta</b> incluye detalles de la embarcación, historial completo y costos — ideal para compradores.
            </div>
          </div>
        ):(
          <div style={{padding:"24px",textAlign:"center"}}>
            <div style={{fontSize:48,marginBottom:12}}>📄</div>
            <div style={{fontSize:16,fontWeight:700,color:"#0f172a",marginBottom:16}}>Reporte Listo</div>
            <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,padding:"16px",textAlign:"left",marginBottom:16}}>
              {[["Embarcación",vessel.name],["Records históricos",vessel.records.length],["Entradas bitácora",vessel.log.length],["Costo total",`$${totalCost.toLocaleString("en-US",{minimumFractionDigits:2})}`],["Tareas activas",vessel.tasks.length],["Tipos incluidos",sel.length]].map(([k,v])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6,paddingBottom:6,borderBottom:"1px solid #f1f5f9"}}><span style={{color:"#64748b"}}>{k}</span><span style={{fontWeight:600}}>{v}</span></div>
              ))}
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"center"}}>
              <button style={{...s.btnPrimary,background:"#16a34a"}}>⬇ Descargar PDF</button>
              <button style={s.btnPrimary}>📧 Enviar por Email</button>
            </div>
          </div>
        )}
        <div style={s.modalFooter}>
          <button style={s.btnOutline} onClick={onClose}>Cancelar</button>
          {!gen
            ?<button style={{...s.btnPrimary,opacity:sel.length===0?0.5:1}} onClick={()=>sel.length>0&&setGen(true)}>Generar ({sel.length} tipo{sel.length!==1?"s":""})</button>
            :<button style={s.btnOutline} onClick={()=>setGen(false)}>← Volver</button>
          }
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{padding:"40px 28px",maxWidth:700,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        {[
          {icon:"☁️",title:"Google Drive",desc:"Integración directa con tu Google Drive. Tus archivos siguen en Google — sin costo adicional de almacenamiento. Recomendado.",tag:"Recomendado",tagColor:"#16a34a"},
          {icon:"📁",title:"Almacenamiento interno",desc:"Sube archivos directamente a NautiTrack. Más simple pero implica costo de almacenamiento en servidor a largo plazo.",tag:"En desarrollo",tagColor:"#d97706"},
        ].map(opt=>(
          <div key={opt.title} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"20px",boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
      ...
      <div style={{marginTop:20,padding:"14px 16px",background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:10,fontSize:13,color:"#0369a1"}}>
        💡 <strong>Recomendación:</strong> Usar Google Drive es más seguro, gratuito hasta 15GB y no depende de los servidores de NautiTrack. Conecta tu cuenta y verás tus carpetas directamente aquí.
      </div>
    </div>
  );
}
...
function ProfileModal({vessel,updateVessel,onClose}) {
  ...
  const planFeatures = {
    Basic:["1 embarcación","Bitácora básica","Tareas ilimitadas","Soporte por email"],
    Pro:  ["Hasta 3 embarcaciones","Bitácora con fotos","Records y reportes","Proveedores","Soporte prioritario"],
    Fleet:["Embarcaciones ilimitadas","Todo lo de Pro","API access","Reportes automáticos","Gerente de cuenta"],
  };
  ...
  return (
    <div style={s.modalOverlay} onClick={onClose}>
      <div style={{...s.modalBox,maxWidth:560}} onClick={e=>e.stopPropagation()}>
        <div style={s.modalHeader}>
          <div style={{fontSize:16,fontWeight:700,color:"#0f172a"}}>👤 Mi Cuenta</div>
          <button onClick={onClose} style={s.modalClose}>✕</button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
          {tab==="profile"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:16,padding:"16px",background:"#f8fafc",borderRadius:10,border:"1px solid #e2e8f0"}}>
                ...
                <div>
                  <div style={{fontWeight:700,fontSize:15,color:"#0f172a"}}>{form.firstName} {form.lastName}</div>
                  <div style={{fontSize:12,color:"#64748b"}}>{form.email}</div>
                  <button style={{...s.btnOutline,padding:"3px 10px",fontSize:11,marginTop:6}}>Cambiar foto</button>
                </div>
              </div>
              ...
              <div><label style={s.label}>Ubicación de la Marina</label><input value={form.marinaAddress||""} onChange={e=>set("marinaAddress",e.target.value)} placeholder="Marina, Muelle, Ciudad, Estado, País" style={s.input}/></div>
            </div>
          )}
          ...
          {tab==="subscription"&&(
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {/* Current plan */}
              <div style={{background:"linear-gradient(135deg,#1d4ed8,#0ea5e9)",borderRadius:12,padding:"20px",color:"#fff"}}>
                ...
                {Object.entries(planFeatures).map(([plan,features])=>(
                  <div key={plan} style={{border:`1.5px solid ${sub.plan===plan?"#0ea5e9":"#e2e8f0"}`,borderRadius:10,padding:"14px 12px",background:sub.plan===plan?"#f0f9ff":"#fff"}}>
                    ...
                    <div style={{fontSize:16,fontWeight:700,color:"#1e293b",marginBottom:10}}>
                      {plan==="Basic"?"$29":plan==="Pro"?"$79":"$149"}<span style={{fontSize:11,color:"#94a3b8"}}>/mes</span>
                    </div>
              ...
              {/* Venezuela pricing note */}
              <div style={{padding:"12px 14px",background:"#fefce8",border:"1px solid #fde68a",borderRadius:8,fontSize:12,color:"#92400e"}}>
                💡 <strong>Pagos en Venezuela:</strong> Aceptamos Zelle, transferencia bancaria USD, pago móvil y efectivo. Contáctanos para opciones de pago locales.
              </div>
            </div>
          )}
        </div>
        ...
        <div style={s.modalFooter}>
          <button style={s.btnOutline} onClick={onClose}>Cancelar</button>
          {tab==="profile"&&<button style={s.btnPrimary} onClick={save}>Guardar Cambios</button>}
        </div>
      </div>
    </div>
  );
      ...
      <div style={{...s.modalBox,maxWidth:720}} onClick={e=>e.stopPropagation()}>
        <div style={s.modalHeader}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button style={s.btnPrimary} onClick={()=>setShowAdd(!showAdd)}>＋ Agregar</button>
            <button onClick={onClose} style={s.modalClose}>✕</button>
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"16px 24px"}}>
          {showAdd&&(
            <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:10,padding:"16px",marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:12,justifyContent:"flex-end"}}>
                <button style={s.btnOutline} onClick={()=>setShowAdd(false)}>Cancelar</button>
                <button style={s.btnPrimary} onClick={addProvider}>Guardar</button>
              </div>
            </div>
          )}
          <table style={s.table}>
...
const s = {
  ...
  photoUpload:{border:"2px dashed #bae6fd",borderRadius:10,padding:"14px",textAlign:"center",background:"#f0f9ff"},
};