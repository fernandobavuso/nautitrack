...
export default function LocationPicker({ value, onChange }) {
  ...
  const useMyLocation = async () => {
    ...
    try {
      ...
      onChange({ lat, lng, label: t("store.myLocation") });
      setQuery(t("store.myLocation"));
      setResults([]);
      setMsg("✓ "+t("store.locDetected"));
    } catch (e) {
      setMsg(t("store.locError"));
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      <button type="button" onClick={useMyLocation} disabled={loading} style={{width:"100%",padding:"11px",background:"#eff6ff",border:"1.5px solid #bae6fd",borderRadius:9,color:"#2563eb",fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
        ...
        {loading ? t("store.detecting") : t("store.myLocation")}
      </button>
      <div style={{fontSize:11,color:"#94a3b8",textAlign:"center",marginBottom:8}}>{t("store.searchCity")}</div>
      <div style={{position:"relative"}}>
        <input value={query} onChange={e=>onType(e.target.value)} placeholder={t("store.cityPh")} style={{width:"100%",padding:"10px 12px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:13,boxSizing:"border-box"}}/>
        {results.length>0 && (
      ...
      {value?.lat && <div style={{fontSize:11,color:"#64748b",marginTop:6}}>📍 {t("store.locSaved")}: {value.label}</div>}
    </div>
  );
}
