    ...
    <div style={{maxWidth:900,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:13,color:"#64748b"}}>{L("Inventario a bordo y alertas de reposición","Onboard inventory and restock alerts")}</div>
        </div>
        ...
        <div>
          {myRequests.length===0&&(
            <div style={{textAlign:"center",padding:"40px 0",color:"#94a3b8"}}>
              <div style={{fontWeight:600}}>{L("No has pedido repuestos","You haven't requested any parts")}</div>
              <div style={{fontSize:12,marginTop:4}}>{L('Usa "Pedir repuesto" para que las tiendas de tu zona te coticen','Use "Request part" so stores in your area can quote you')}</div>
            </div>
          )}
                ...
                <div key={r.id} style={{background:"#fff",border:`1px solid ${pendingApproval?"#fde68a":"#e2e8f0"}`,borderRadius:12,padding:14,opacity:resolved?0.7:1}}>
                  {pendingApproval&&(
                    <div style={{background:"#fffbeb",border:"1px solid #fde68a",borderRadius:8,padding:"10px 12px",marginBottom:10}}>
                      ...
                      <div style={{display:"flex",gap:8}}>
                        <button onClick={()=>approveRequest(r)} style={{flex:1,padding:"7px",background:"#16a34a",border:"none",borderRadius:7,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>{L("Aprobar y publicar","Approve and publish")}</button>
                        <button onClick={()=>rejectRequest(r)} style={{flex:1,padding:"7px",background:"#fff",border:"1px solid #fecaca",borderRadius:7,color:"#dc2626",fontSize:12,fontWeight:700,cursor:"pointer"}}>{L("Rechazar","Reject")}</button>
                      </div>
                    </div>
                  )}
                  <div style={{display:"flex",gap:12,marginBottom:resp.length?10:0}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:"#0f172a"}}>{r.item_name}{r.urgent&&<span style={{marginLeft:8,fontSize:10,background:"#fee2e2",color:"#dc2626",padding:"2px 8px",borderRadius:10,fontWeight:700}}>{L("URGENTE","URGENT")}</span>}</div>
                    ...
                    {!pendingApproval&&(resolved
                      ? <button onClick={()=>reopenRequest(r.id)} style={{background:"none",border:"1px solid #e2e8f0",borderRadius:8,padding:"5px 10px",fontSize:11,color:"#64748b",cursor:"pointer",fontWeight:600,height:"fit-content"}}>{L("Reabrir","Reopen")}</button>
                      : <button onClick={()=>closeRequest(r.id)} style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"5px 10px",fontSize:11,color:"#16a34a",cursor:"pointer",fontWeight:700,height:"fit-content"}}>{L("Cerrar","Close")}</button>
                    )}
                  </div>
                      ...
                      {ordered.map(rp=>{
                        ...
                        <div key={rp.id} style={{background:isWinner?"#f0fdf4":isBest&&!resolved?"#eff6ff":"#f8fafc",border:isWinner?"1px solid #bbf7d0":isBest&&!resolved?"1px solid #bfdbfe":"1px solid transparent",borderRadius:8,padding:"10px 12px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                            <div style={{flex:1}}>
                              <div style={{fontSize:13,fontWeight:700,color:"#0f172a"}}>
                                ...
                                {isBest&&!resolved&&rp.response_type==="have"&&<span style={{marginLeft:6,fontSize:9,background:"#dbeafe",color:"#2563eb",padding:"2px 7px",borderRadius:10,fontWeight:700}}>{L("Mejor precio","Best price")}</span>}
                                {isWinner&&<span style={{marginLeft:6,fontSize:9,background:"#dcfce7",color:"#16a34a",padding:"2px 7px",borderRadius:10,fontWeight:700}}>{L("Elegida","Chosen")}</span>}
                              </div>
                          ...
                          {!resolved&&rp.response_type==="have"&&rp.price&&(
                            <button onClick={()=>chooseWinner(r,rp)} style={{width:"100%",marginTop:8,padding:"7px",background:"linear-gradient(120deg,#2563eb,#0ea5e9)",border:"none",borderRadius:7,color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>{L("Elegir esta tienda","Choose this store")}</button>
                          )}
                          {/* Coordinación tras elegir la tienda */}
                          {isWinner&&(
                            <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid #bbf7d0"}}>
                              <div style={{fontSize:11,fontWeight:700,color:"#15803d",marginBottom:6}}>{L("Coordina la entrega con la tienda","Coordinate delivery with the store")}</div>
                              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                                {rp.store?.store_phone ? (
                                  ...
                                  <span style={{fontSize:11,color:"#94a3b8"}}>{L("La tienda no registró teléfono","The store didn't provide a phone")}</span>
                                )}
                              </div>
                  ...
                  {resp.length===0&&!resolved&&<div style={{fontSize:11,color:"#94a3b8",fontStyle:"italic"}}>{L("Esperando respuestas de las tiendas...","Waiting for store responses...")}</div>}
                </div>
              );
                    })}
                  </div>
                        ...
                        <div key={it.id} style={{padding:"12px 16px",borderBottom:"1px solid #f4f7fa",background:low?"#fff8f8":"#fff"}}>
                          <div style={{display:"flex",alignItems:"center",gap:12}}>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:13,fontWeight:700,color:"#0f172a"}}>{it.name}{low&&<span style={{marginLeft:8,fontSize:10,background:"#fee2e2",color:"#dc2626",padding:"2px 8px",borderRadius:10,fontWeight:700}}>{L("Reponer","Restock")}</span>}</div>
                            ...
                            <button onClick={()=>openEdit(it)} style={{background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:13,fontWeight:600}}>{L("Editar","Edit")}</button>
                            <button onClick={()=>{ if(confirm(`¿Eliminar "${it.name}" del inventario?`)) del(it.id); }} style={{background:"none",border:"none",cursor:"pointer",color:"#dc2626",fontSize:13,fontWeight:600}}>{L("Eliminar","Delete")}</button>
                          </div>
                        </div>
                      );
                    })}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:16}} onClick={()=>setEditing(null)}>
          <div style={{background:"#fff",borderRadius:16,padding:22,maxWidth:440,width:"100%",maxHeight:"88vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              ...
              {editing!=="new"&&<button onClick={()=>{del(editing);setEditing(null);}} style={{background:"none",border:"none",cursor:"pointer",color:"#dc2626",fontSize:12,fontWeight:700}}>{L("Eliminar","Delete")}</button>}
            </div>
            <div style={{marginBottom:10}}>
              <label style={lbl}>{L("Nombre del repuesto *","Part name *")}</label>
              <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder={L("Ej: Filtro de aceite Fleetguard FF5052","e.g. Fleetguard FF5052 oil filter")} style={inp}/>
            </div>
            <div style={{display:"flex",gap:8,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <input value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})} placeholder={L("Ej: Fleetguard","e.g. Fleetguard")} style={inp}/>
              </div>
              <div style={{flex:1}}>
                <label style={lbl}>{L("N° de parte","Part No.")}</label>
                <input value={form.part_num} onChange={e=>setForm({...form,part_num:e.target.value})} placeholder={L("FF5052","FF5052")} style={inp}/>
              </div>
            </div>
            <div style={{display:"flex",gap:8,marginBottom:10}}>
              <div style={{flex:1}}>
                <label style={lbl}>{L("Cantidad actual","Current quantity")}</label>
                <input type="number" value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})} placeholder="0" style={inp}/>
              </div>
              <div style={{flex:1}}>
                <label style={lbl}>{L("Mínimo (alerta)","Minimum (alert)")}</label>
                <input type="number" value={form.min_quantity} onChange={e=>setForm({...form,min_quantity:e.target.value})} placeholder="2" style={inp}/>
              </div>
              <div style={{width:110}}>
                <label style={lbl}>{L("Unidad","Unit")}</label>
            ...
            <div style={{display:"flex",gap:8,marginBottom:10}}>
              <div style={{width:150}}>
                <label style={lbl}>{L("Ubicación","Location")}</label>
                <select value={form.location_type} onChange={e=>setForm({...form,location_type:e.target.value})} style={inp}>
                  {LOCATIONS.map(l=><option key={l}>{l}</option>)}
                </select>
              </div>
              <div style={{flex:1}}>
                <label style={lbl}>{L("Detalle de ubicación","Location detail")}</label>
                <input value={form.location} onChange={e=>setForm({...form,location:e.target.value})} placeholder={L("Ej: gaveta 2, lado estribor","e.g. drawer 2, starboard side")} style={inp}/>
              </div>
            </div>
            <div style={{marginBottom:10}}>
              <label style={lbl}>{L("¿Dónde se compró?","Where was it bought?")}</label>
              <input value={form.bought_at} onChange={e=>setForm({...form,bought_at:e.target.value})} placeholder={L("Ej: Náutica El Morro, Lechería","e.g. Marine Supply, Miami")} style={inp}/>
            </div>
            <div style={{marginBottom:16}}>
              <label style={lbl}>{L("Notas","Notes")}</label>
              <input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder={L("Opcional","Optional")} style={inp}/>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setEditing(null)} style={{flex:1,padding:"11px",background:"#f1f5f9",border:"none",borderRadius:8,color:"#475569",fontSize:13,fontWeight:600,cursor:"pointer"}}>{L("Cancelar","Cancel")}</button>
              <button onClick={save} style={{flex:2,padding:"11px",background:"linear-gradient(120deg,#2563eb,#0ea5e9)",border:"none",borderRadius:8,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>{L("Guardar","Save")}</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:16}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:16,padding:22,maxWidth:420,width:"100%",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:10}}>
          <label style={lbl}>{L("¿Qué necesitas? *","What do you need? *")}</label>
          <input value={form.item_name} onChange={e=>setForm({...form,item_name:e.target.value})} placeholder={L("Ej: Filtro de aceite Fleetguard FF5052","e.g. Fleetguard FF5052 oil filter")} style={inp}/>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:10}}>
          <div style={{flex:1}}>
            <label style={lbl}>{L("Categoría","Category")}</label>
          ...
          <div style={{flex:1}}>
            <label style={lbl}>{L("N° de parte","Part No.")}</label>
            <input value={form.part_num} onChange={e=>setForm({...form,part_num:e.target.value})} placeholder={L("Opcional","Optional")} style={inp}/>
          </div>
        </div>
        <div style={{marginBottom:10}}>
          <label style={lbl}>{L("Detalles","Details")}</label>
          <textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} rows={2} placeholder={L("Marca del motor, año, cantidad que necesitas...","Engine brand, year, quantity you need...")} style={{...inp,resize:"vertical"}}/>
        </div>
        <div style={{marginBottom:10}}>
          <label style={lbl}>{L("¿Dónde buscar?","Where to search?")}</label>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setForm({...form,scope:"city"})} style={{flex:1,padding:"9px",borderRadius:8,border:"1.5px solid",cursor:"pointer",fontSize:12,fontWeight:700,background:form.scope==="city"?"#eff6ff":"#fff",borderColor:form.scope==="city"?"#2563eb":"#e2e8f0",color:form.scope==="city"?"#2563eb":"#64748b"}}>{L("En mi ciudad","In my city")}</button>
            <button onClick={()=>setForm({...form,scope:"national"})} style={{flex:1,padding:"9px",borderRadius:8,border:"1.5px solid",cursor:"pointer",fontSize:12,fontWeight:700,background:form.scope==="national"?"#eff6ff":"#fff",borderColor:form.scope==="national"?"#2563eb":"#e2e8f0",color:form.scope==="national"?"#2563eb":"#64748b"}}>{L("Toda Venezuela","Nationwide")}</button>
          </div>
        </div>
        <div style={{marginBottom:10}}>
          <label style={lbl}>{L("Foto del repuesto (recomendado)","Part photo (recommended)")}</label>
        ...
        {isCaptain&&(
          <div style={{marginBottom:10}}>
            <label style={lbl}>{L("Costo estimado (USD)","Estimated cost (USD)")}</label>
            <input type="number" value={form.est_amount} onChange={e=>setForm({...form,est_amount:e.target.value})} placeholder={L("¿Cuánto crees que cuesta?","How much do you think it costs?")} style={inp}/>
            {needsApproval&&(
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,padding:"10px 12px",background:"#fff5f5",border:"1px solid #fecaca",borderRadius:8}}>
          ...
          <label htmlFor="urgent" style={{fontSize:12,color:"#b91c1c",cursor:"pointer",fontWeight:600}}>{L("Urgente — barco varado o necesito esto ya","Urgent — boat down or I need this now")}</label>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={onClose} style={{flex:1,padding:"11px",background:"#f1f5f9",border:"none",borderRadius:8,color:"#475569",fontSize:13,fontWeight:600,cursor:"pointer"}}>{L("Cancelar","Cancel")}</button>
