      ...
      <div style={box} onClick={e=>e.stopPropagation()}>
        <div style={{padding:"20px 24px",borderBottom:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{fontSize:12,color:"#64748b"}}>{L("Elige el plan que se ajusta a ti","Choose the plan that fits you")}</div>
          </div>
        ...
        {FOUNDER_ACCESS && (
          <div style={{margin:"16px 24px 0",background:"linear-gradient(120deg,#0a2540,#1e3a8a)",borderRadius:12,padding:"12px 16px"}}>
            <div style={{fontSize:13,fontWeight:800,color:"#fff",fontFamily:"'Sora',system-ui,sans-serif",marginBottom:2}}>{L("Estás en Acceso de Fundador","You're on Founder Access")}</div>
            <div style={{fontSize:11,color:"#bfdbfe",lineHeight:1.4}}>{L("Ahora mismo tienes todas las funciones desbloqueadas, gratis. Estos son los planes que estarán disponibles más adelante — como fundador tendrás condiciones especiales.","Right now you have every feature unlocked, free. These are the plans that will be available later — as a founder you'll get special terms.")}</div>
          </div>
        )}
        ...
        <div style={{padding:"20px 24px",overflowY:"auto"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",marginBottom:24}}>
            <div style={{display:"inline-flex",background:"#f1f5f9",borderRadius:10,padding:4}}>
              <button onClick={()=>setYearly(false)} style={toggleBtn(!yearly)}>{L("Mensual","Monthly")}</button>
              <button onClick={()=>setYearly(true)} style={toggleBtn(yearly)}>{L("Anual","Yearly")}< span style={{fontSize:10,color:yearly?"#bbf7d0":"#16a34a",fontWeight:700}}>2 meses gratis</span></button>
            </div>
          </div>
            ...
            {["free","pro","fleet"].map(key=>{
              ...
              return (
                <div key={key} style={{
                  ...
                  {highlight&&<div style={{position:"absolute",top:-10,left:"50%",transform:"translateX(-50%)",background:"#1d4ed8",color:"#fff",fontSize:10,fontWeight:700,padding:"3px 12px",borderRadius:12}}>{L("MÁS POPULAR","MOST POPULAR")}</div>}
                  ...
                  {isCurrent
                    ? <div style={{padding:"10px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,color:"#16a34a",fontSize:13,fontWeight:700,textAlign:"center"}}>{L("Tu plan actual","Your current plan")}</div>
                    : key==="free"
                      ? <div style={{padding:"10px",background:"#f8fafc",borderRadius:8,color:"#94a3b8",fontSize:13,fontWeight:600,textAlign:"center"}}>{L("Gratis siempre","Free forever")}</div>
